...
class CSVLogger:
    ...
    def get_f1_column_name(index):
        return CSVLogger.col_name(CSVLogger.F1Test, index)
    ...
    def write_evaluation_results(self, current_epoch, result_test, result_train, avg_cost, avg_acc):
        ...
        assert(isinstance(result_test, TwoClassEvalResult))
        assert(isinstance(result_train, TwoClassEvalResult))

        i = current_epoch

        self.write_value(self.get_f1_column_name(i), result_test.Result[TwoClassEvalResult.C_F1],
                         debug=DebugKeys.LoggerEvaluationF1)
        self.write_value(self._get_f1p_column_name(i), result_test.Result[TwoClassEvalResult.C_F1_POS], True)
        self.write_value(self._get_f1n_column_name(i), result_test.Result[TwoClassEvalResult.C_F1_NEG], True)
        self.write_value(self._get_pp_column_name(i), result_test.Result[TwoClassEvalResult.C_POS_PREC], True)
        self.write_value(self._get_np_column_name(i), result_test.Result[TwoClassEvalResult.C_NEG_PREC], True)
        self.write_value(self._get_pr_column_name(i), result_test.Result[TwoClassEvalResult.C_POS_RECALL], True)
        self.write_value(self._get_nr_column_name(i), result_test.Result[TwoClassEvalResult.C_NEG_RECALL], True)
        self.write_value(self._get_f1_train_column_name(i), result_train.Result[TwoClassEvalResult.C_F1],
